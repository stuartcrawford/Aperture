	...
	<head>
		...
		<script>
      	$(function() {
      		...
      		//Bind Footable to Vehicle Table
      		$('#Vehicle, #Vehicle-inactive').footable({
			...
			//Force Vehicle Breakpoint based on Parent Width
			$('#resize1').click(function() {
				...
				setTimeout(function() {
					...
					$('#Vehicle')[0].footable.resize();
					
				}, 1);
			});
			
			//Force Vehicle-Inactive Breakpoint based on Parent Width
			$('#resize2').click(function() {
				...
				setTimeout(function() {
					...
					$('.footable-container').css({width:w});
					$('#Vehicle-inactive')[0].footable.resize();
		...
		<script>
			...
			$(function() {    
   	 			$('#filter').change(function() {
   	 				...
   	 				$('#Vehicle tbody tr').hide();
   	 				$('#Vehicle tbody tr').removeClass('footable-detail-show');
   	 				
   	 				if ($(this).val() == '') {
   	 					$('#Vehicle tbody tr:not(.footable-row-detail)').show();	
   	 				}
   	 				else { 
   	 					...
   	 					if (column == 'All') {
   	 						$("#Vehicle tbody tr:containsIgnoreCase('" + $(this).val() + "')").show();
   	 					}
   	 					else if (column == 'AssetNo' || column == 'AssetGroupNo'){
	        				$("#Vehicle tbody td."+ column +":containsExact('" + $(this).val() + "')").parent().show();
        				}
        				else {
        					$("#Vehicle tbody td."+ column +":containsIgnoreCase('" + $(this).val() + "')").parent().show();
        				}
        			}
    			});
    		});
		...
		<script>
			...
			{	
				...
				$.ajax({
					url: "<?php fetchdir($php); ?>manage_vehicles_tab3.php",
					data: 
					{
						CSRFToken: CSRFToken,  
						action: action,
				...
				$.ajax({
					url: "<?php fetchdir($php); ?>manage_vehicles_tab4.php",
					data:  
					{
						CSRFToken: CSRFToken,  
						action: action,
				...
				<form class="custom">
					...
					<select id="customDropdown" class="large">
						<option value="All">All</option>
  						...
  						<option value="MOTExpiry">MOT Expiry</option>
  						<option value="TaxExpiry">Tax Expiry</option>
				...
				<div class="section-container tabs" data-section data-options="deep_linking: false">
					...
					<section id="section1" class="section" style="">
						...
						<div class="content" data-slug="panel1" style="">		
							<!-- Vehicle Table -->
							<?php include realpath(dirname(__FILE__)."/../../php/manage_vehicles_tab1.php"); ?>
							<!-- End Vehicle Table -->	
						</div>
					</section>
					<!-- End Tab 1 -->
					...
					<section id="section2" class="section" style="">
						...
						<div class="content" data-slug="panel2" style="">
							<?php include realpath(dirname(__FILE__)."/../../php/manage_vehicles_tab2.php"); ?>	
						</div>
					</section>
					<!-- End Tab 2 -->
					...
					<section id="section3" class="section" style="">
						...
						<div class="content" data-slug="panel3" style="">
							<h3>Please Select a Vehicle</h3>
							<p>You will need to have selected a vehicle from the View All tab to see the information in this section</p>
						</div>
					</section>
					<!-- End Tab 3 -->
					...
					<section id="section5" class="section" style="">
						...
						<div class="content" data-slug="panel5" style="">
							<?php include realpath(dirname(__FILE__)."/../../php/manage_vehicles_tab5.php"); ?>	
						</div>
					</section>
					<!-- End Tab 5 -->
