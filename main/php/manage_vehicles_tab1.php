<?php
	...
	if ($_SESSION['User']['AccessLvl'] == 1 || $_SESSION['User']['AccessLvl'] == 2) {
		...
		//Select Query
		$stmt = $conn->prepare("SELECT * FROM Vehicle, Asset, Asset_Group, Location WHERE Vehicle.AssetNo = Asset.AssetNo AND Asset.AssetGroupNo = Asset_Group.AssetGroupNo AND Asset.Location = Location.LocationID AND Asset.Active = 1");
		$stmt->execute();
		...
		$html_table = '<table id="Vehicle" data-filter="#filter" class="footable">' . "\n"  . '<thead>';
		...
		echo $html_table;
			...
			echo '<p>There are no active Vehicles at this time</p>';
		}	
