<?php
	...
	//View_Manage_Inactive_Locations
	...
	if ($_SESSION['User']['AccessLvl'] == 1) {
		...
		//Select Query
		$stmt = $conn->prepare("SELECT * FROM Location WHERE Active = 0");
		$stmt->execute();
		...
		$html_table = '<table id="Location-inactive" data-filter="#filter" class="footable">' . "\n"  . '<thead>';
		...
		echo $html_table;
			...
			echo '<p>There are no inactive Locations at this time</p>';
		}
