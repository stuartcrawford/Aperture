<?php
	...
	if ($_SESSION['User']['AccessLvl'] == 1 || $_SESSION['User']['AccessLvl'] == 2) {
		...
		//Select Query
		$stmt = $conn->prepare("
						SELECT  bh.ActualPickUpDate AS Date, l.Add1 AS Location,
								bh.StartMileage AS Mileage, ag.Manufacturer, ag.Definition,
							...
							ON bh.PickupLocation = l.LocationID
							WHERE bh.ActualPickUpDate IS NOT NULL
							ORDER BY Date DESC
							");
		$stmt->execute();
		...
		$html_table = '<table id="Location-D" data-filter="#filter" class="footable">' . "\n"  . '<thead>';
