<?php
	...
	if ($_SESSION['User']['AccessLvl'] == 1) {
        ...
        SQ1, SQ2, SQA1, SQA2 FROM User WHERE Status = 1");
		$stmt->execute();
		...
		$html_table = '<table id="Account" data-filter="#filter" class="footable">' . "\n"  . '<thead>';
		...
		foreach( $rslt as $row ) {	
			...
			$html_table .= '<td class="Title" data-hide="phone">'.$row['Title'].'</td>';
			$html_table .= '<td class="DOB" data-hide="phone,tablet">'.$row['DOB'].'</td>';
			$html_table .= '<td class="FirstName" data-hide="phone">'.$row['FirstName'].'</td>';
			$html_table .= '<td class="LastName" data-hide="phone">'.$row['LastName'].'</td>';
			$html_table .= '<td class="Add1" data-hide="phone,tablet">'.$row['Add1'].'</td>';
			$html_table .= '<td class="Add2" data-hide="phone,tablet">'.$row['Add2'].'</td>';
			$html_table .= '<td class="Add3" data-hide="phone,tablet">'.$row['Add3'].'</td>';
			$html_table .= '<td class="Postcode" data-hide="phone,tablet">'.$row['Postcode'].'</td>';
			$html_table .= '<td class="Phone" data-hide="phone,tablet">'.$row['Telephone'].'</td>';
			$html_table .= '<td class="Phone" data-hide="phone,tablet">'.$row['Mobile'].'</td>';
			$html_table .= '<td class="LicenseNo" data-hide="phone,tablet">'.$row['LicenseNo'].'</td>';
			$html_table .= '<td class="NextLogin" data-hide="phone,tablet">'.$row['NextLogin'].'</td>';
			$html_table .= '<td class="LoginAttempts" data-hide="phone,tablet">'.$row['LoginAttempts'].'</td>';
			...
			if ($row['AccessLvl'] == 3) { $html_table .= '<td class="SQ1" data-hide="phone,tablet">'.$row['SQ1'].'</td>'; }
			if ($row['AccessLvl'] == 3) { $html_table .= '<td class="SQ2" data-hide="phone,tablet">'.$row['SQ2'].'</td>'; }
			if ($row['AccessLvl'] == 3) { $html_table .= '<td class="SQA1" data-hide="phone,tablet">'.$row['SQA1'].'</td>'; }
			if ($row['AccessLvl'] == 3) { $html_table .= '<td class="SQA2" data-hide="phone,tablet">'.$row['SQA2'].'</td>'; }
			$html_table .= '</tr>';
		...
		echo $html_table;
			...
			echo '<p>There are no live Accounts at this time</p>';
		}
	...
	// Staff View
		...
		$stmt = $conn->prepare("SELECT UserID, Status, CreationDate, AccessLvl, Title, Email, DOB, FirstName, 
        LastName, Add1, Add2, Add3, Postcode, Telephone, Mobile, LicenseNo, NextLogin, LoginAttempts, 
        SQ1, SQ2, SQA1, SQA2 FROM User WHERE Status = 1 AND AccessLvl = 3");
		$stmt->execute();
		$rslt = $stmt->fetchAll(PDO::FETCH_ASSOC);	
		...
		echo $html_table;
			...
			echo '<h3>No Results</h3>';
			echo '<p>There are no live Customers at this time</p>';
		}
	}
?>