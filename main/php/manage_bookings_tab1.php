<?php
	...
	if ($_SESSION['User']['AccessLvl'] == 1 || $_SESSION['User']['AccessLvl'] == 2) {
		...
		//Select Query
		$stmt = $conn->prepare("SELECT * FROM Booking_Header, Location WHERE Status NOT IN(0) AND Booking_Header.PickupLocation = Location.LocationID");
		$stmt->execute();
		...
		$html_table = '<table id="Booking_Head" data-filter="#filter" class="footable">' . "\n"  . '<thead>';
		...
		foreach( $rslt as $row ) {	
			...
			if($row['Status'] == 1) { $status = 'Active'; } else if($row['Status'] == 2) { $status = 'Complete'; } else if($row['Status'] == 3) { $status = 'Missed'; } else if($row['Status'] == 4) { $status = 'Reservation'; } 
			...
			$html_table .= '<td data-sort-ignore="true"><div id="select_record'.$row['BookingHeadID'].'" class="select_record"><a href="javascript:selectrecord(\''.$row['BookingHeadID'].'\');">Select</a></div></td>';
			$html_table .= '</tr>';	
		...
		echo $html_table;
			...
			echo '<p>There are no live Bookings at this time</p>';
		}
	}
