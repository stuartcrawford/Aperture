...
if ($_POST['action'] == 'activebookings') {
	...
	if ($_SESSION['User']['AccessLvl'] == 1 || $_SESSION['User']['AccessLvl'] == 2) {
		...
		//Select Query
		$selectedvehicle = $_POST['record_id'];
		$stmt = $conn->prepare("SELECT * FROM Location, Booking_Header WHERE AssetNo = :selectedvehicle AND PickupLocation = Location.LocationID AND Booking_Header.Status = 1");
		$stmt->execute(array('selectedvehicle' => $selectedvehicle));
		...
		if ($rslt) {
			...
			$html_table .= '<th data-type="numeric">Vehicle ID</th>';
			...
			foreach( $rslt as $row ) {
				...
				$html_table .= '<td class="Location" data-hide="phone,tablet">'.$row['VehicleCost'].'</td>';
