<?php
	...
	if ($_SESSION['User']['AccessLvl'] == 1 || $_SESSION['User']['AccessLvl'] == 2) {
		...
		//Select Query
		$stmt = $conn->prepare("SELECT * FROM Optional_Extra, Asset, Asset_Group, Location WHERE Optional_Extra.AssetNo = Asset.AssetNo AND Asset.AssetGroupNo = Asset_Group.AssetGroupNo AND Asset.Location = Location.LocationID AND Asset.Active = 0");
		$stmt->execute();
		...
		$html_table = '<table id="Optional_Extra-inactive" data-filter="#filter" class="footable">' . "\n"  . '<thead>';
		...
		echo $html_table;
			...
			echo '<p>There are no inactive Optional Extras at this time</p>';
		}
