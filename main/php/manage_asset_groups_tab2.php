<?php
	...
	//View_Manage_Asset_Group-inactive
	//@Author: Stuart Crawford
	//@Date: 01.05.2013
	if(isset($_POST['extragroup']) || isset($_POST['vehiclegroup'])) { include 'global.php'; }
	dbconnect();
	
	if($_POST['extragroup'] == 'hide' && $_POST['vehiclegroup'] == 'hide') { echo '<h3>Please Filter on an Asset Group Type</h3>
	...
	if ($_SESSION['User']['AccessLvl'] == 1) {
			...
			if($_POST['assetgroup'] == 'hide' && $_POST['extragroup'] == 'hide' || !isset($_POST['assetgroup']) && !isset($_POST['extragroup']) ) {
				...
				//Select Query
				$stmt = $conn->prepare("SELECT * FROM Asset_Group WHERE Active = 0 AND Manufacturer IS NOT NULL");
				$stmt->execute();
				...
				//Select Query
				$stmt = $conn->prepare("SELECT * FROM Asset_Group WHERE Active = 0 AND Manufacturer IS NULL");
				$stmt->execute();
				$rslt = $stmt->fetchAll(PDO::FETCH_ASSOC);
				if (!$rslt)
				exit;
		...
		$html_table = '<table id="Asset_Group-inactive" data-filter="#filter" class="footable">' . "\n"  . '<thead>';
		...
		echo $html_table;
			...
			echo '<p>There are no inactive Asset Groups at this time</p>';
		}
		}
